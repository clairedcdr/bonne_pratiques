...
print(summarise(df2, length(unique(unlist(df2$cs3[!is.na(df2$cs3)])))))
print("Nombre de professions :")
print(summarise(df2, length(unique(unlist(df2$cs2[!is.na(df2$cs2)])))))
print("Nombre de professions :")
print(summarise(df2, length(unique(unlist(df2$cs1[!is.na(df2$cs1)])))))

summarise(group_by(df2, df2$aged), n())
...
df2 %>%
  select(aged) %>%
  ggplot(.) +
  geom_histogram(aes(x = 5 * floor(as.numeric(df2$aged) / 5)), stat = "count")
